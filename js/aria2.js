  ...
  return {
    ...
    pause_all: function() {
      ARIA2.request("pauseAll", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "暂停所有任务. 请等待 BitTorrent 服务器响应.", 2000);
        }
      );
    },
    ...
    unpause_all: function() {
      ARIA2.request("unpauseAll", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "开始所有任务", 2000);
        }
      );
    },
    ...
    purge_download_result: function() {
      ARIA2.request("purgeDownloadResult", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "移除所有 已完成/错误/已删除的下载任务.", 2000);
        }
      );
    },
      ...
      ARIA2.request("changeGlobalOption", [options],
        function(result) {
          if (!result.result)
            ...
            main_alert("alert-success", "保存完成", 2000);
        }
      );
    },
    ...
    auto_refresh: function(interval) {
      ...
      interval_id = window.setInterval(function() {
        ...
        if (select_lock) {
          if (need_refresh) {
            main_alert("", "任务列表已经改变.单击'刷新'按钮以更新任务列表.");
          }
        } else {
          if (need_refresh) {
            ARIA2.refresh();
